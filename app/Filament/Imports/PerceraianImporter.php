...
class PerceraianImporter extends Importer
{
    ...
    public static function getColumns(): array
    {
        return [
            ...
            ImportColumn::make('nama_saksi2')
                ->requiredMapping()
                ->rules(['required', 'max:255']),
            ImportColumn::make('status_perceraian')
                ->requiredMapping()
                ->rules(['required', 'max:255']),
    ...
    public function resolveRecord(): ?Perceraian
    {
        // return Perceraian::firstOrNew([
        //     // Update existing records, matching them by `$this->data['column_name']`
        //     'email' => $this->data['email'],
        // ]);

        return new Perceraian();
    }

    public static function getCompletedNotificationBody(Import $import): string
    {
        $body = 'Your perceraian import has completed and ' . number_format($import->successful_rows) . ' ' . str('row')->plural($import->successful_rows) . ' imported.';
